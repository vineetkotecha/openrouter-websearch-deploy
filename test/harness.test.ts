import{describe,it,expect}from"vitest";import{SearchHarness,MemoryStore}from"../src/core/harness.js";import{HeuristicMandateWriter}from"../src/core/mandate.js";import{SearchRequestSchema}from"../src/contracts/search.js";const c:any={SEARCH_TIMEOUT_MS:1000};const base={query:"best quiet laptop",tenant_id:"t",context:[],permissions:{may_pull_context:false,may_ask_user:false,may_retain:false,may_learn:false,scopes:[]},limits:{latency_ms:1000,max_provider_calls:3,max_results:10}};describe("harness",()=>{it("runs without keys",async()=>{const h=new SearchHarness(c,new HeuristicMandateWriter(),[],new MemoryStore());const r=await h.search(SearchRequestSchema.parse(base));expect(r.status).toBe("complete");if(r.status==="complete")expect(r.limitations[0]).toMatch(/No provider/) });it("keeps mandates internal and never asks the end user",async()=>{const h=new SearchHarness(c,new HeuristicMandateWriter(),[],new MemoryStore());const r=await h.search(SearchRequestSchema.parse({...base,query:"find delivery near me",permissions:{...base.permissions,may_ask_user:true}}));expect(r.status).toBe("complete");expect("mandate" in r).toBe(false);expect("question" in r).toBe(false)})});

describe("contracts",()=>{it("rejects tenantless requests",()=>{expect(SearchRequestSchema.safeParse({query:"x"}).success).toBe(false)});it("does not retain without permission",async()=>{const h=new SearchHarness(c,new HeuristicMandateWriter(),[],new MemoryStore());const r=await h.search(SearchRequestSchema.parse(base));expect(r.status==="complete"&&r.retention_until).toBeUndefined()})});
...
import { SearchRequestSchema as _SRS } from "../src/contracts/search.js";
import { it as _it, expect as _ex } from "vitest";
